...
export default function Generalcbt() {
  ...
  const [candidateSubjects, setCandidateSubjects] = useState(location.state && location.state.subjects);
  const [selectedSubject, setSelectedSubject] = useState(location.state && location.state.subjects);
  const [SubjectID, setSubjectID] = useState(location.state && location.state.subjectsID);
 ...
 // 120 minutes in seconds
  ...
  const fetchData = async (examID) => {
    try {
      ...
      const questionsResponse = await axios.get(`/api/quizequestions/${SubjectID}`);
  ...
  const handleAnswerSelection = (subjId, questionId, selectedOption, candid, examID,ch=null) => {
    // Check if the question has already been answered
    const existingAnswerIndex = answered.findIndex(answer => answer.qid === questionId);
  
    if (existingAnswerIndex !== -1) {
      // If an existing answer is found, update it locally
      const updatedAnswers = answered.map((answer, index) =>
    ...
    setisOptionD(selectedOption === 'D');
      ...
      console.log('Selected option updated locally.');
    } else {
      // If no existing answer is found, insert a new record locally
      const newAnswer = {
        qid: questionId,
        selectedOption: selectedOption,
        subjid: subjId,
      };
      const updatedAnswers = [...answered, newAnswer];
      setAnswered(updatedAnswers);
      ...
      updateAnswers(examID, candid);
      console.log('Selected option added locally.');
    }
  };
...
const handleCompletedExam = () => {
  ...
  //   submitAnswer(answered);
    // Wait for the state to update before navigating or logging
    // Navigate or log inside a useEffect hook or use async/await
 // console.log(results); // This may not reflect the updated results immediately
  navigate('/quizcompleted', { state: { result: results , questions: questions} });
            ...
            <div key={option}>
              ...
              <span style={{ display: 'inline-block', marginRight: 5 }}>
                <input
                  ...
                  onChange={() => handleAnswerSelection(selectedSubject.subjID, selectedQuestion.id, option,regNo,examID,option)}
                />
              </span>

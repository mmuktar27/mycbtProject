...
const AcademicReports = () => {
  ...
  const generateReport = async () => {
    try {
      ...
      const response = await axios.get('/api/reports/academic', { params });
  ...
  const { exporting, handleExcel, handleCSV, handlePDF } = useReportExport({
    rows, columns: COLUMNS, formatCell, reportType: 'academic', reportLabel: 'Academic Reports', showDialog
  });

  const summaryCards = summary
    ? [
        { label: 'Average Score', value: summary.averageScore },
        { label: 'Pass', value: summary.passCount },
        { label: 'Fail', value: summary.failCount }
      ]
    : [];
